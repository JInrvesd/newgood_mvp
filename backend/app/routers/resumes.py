...
async def create_resume(payload: ResumeCreate):
    ...
    data = {"id": resume_id, "form_data": payload.form_data, "status": "draft"}
...
async def upload_docx(file: UploadFile = File(...)):
    """DOCX 업로드 → 텍스트 추출 → LLM 구조화 → form_data 반환"""
    ...
    if llm_data:
        # regex 추출값이 더 정확할 수 있으므로 LLM이 빈 값으로 둔 경우에만 보완
        personal = llm_data.get("personal", {})
        for key in ("phone", "email", "linkedinUrl"):
            if not personal.get(key) and basic_data["personal"].get(key):
                personal[key] = basic_data["personal"][key]
...
async def get_analysis_result(uuid: str):
    ...
    return {"resume": resume.data, "analysis": analysis.data[0] if analysis.data else None}
